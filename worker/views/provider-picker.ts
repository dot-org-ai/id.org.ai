...
export interface ProviderPickerOptions {
  /**
   * Whether upstream Microsoft federation is configured. When true the
   * Microsoft button goes to id.org.ai's OWN Entra app
   * (`/federation/microsoft/start`) rather than the AuthKit fallback — that is
   * the path that gets Ping-federated corporate tenants (zebra.com) through,
   * and the path that can fall back to email codes when a tenant blocks
...
export function renderProviderPicker(continueUrl: string, options: ProviderPickerOptions = {}): Response {
  ...
  const providers: Array<{ id: string; name: string; icon: string; href?: string }> = [
  ...
  if (options.microsoftFederation) {
    const microsoft = providers.find((p) => p.name === 'Continue with Microsoft')
    if (microsoft) microsoft.href = '/federation/microsoft/start'
  }
