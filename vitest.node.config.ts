/// <reference types="vitest" />
export default {
  test: {
    include: [
      'test/cli.test.ts',
      'test/oauth-types.test.ts',
      'test/oauth-storage.test.ts',
      'test/oauth-pkce.test.ts',
      'test/jwt-signing-parity.test.ts',
      'test/oauth-jwt-verify.test.ts',
      'test/oauth-consent.test.ts',
      'test/oauth-guards.test.ts',
      'test/oauth-dev.test.ts',
      'test/oauth-server.test.ts',
      'test/oauth-stripe.test.ts',
      'test/claim-client.test.ts',
      'test/provision-storage.test.ts',
      'test/cli-provision.test.ts',
      'test/cli-claim.test.ts',
      'test/workos-refresh.test.ts',
      'test/foundation-result.test.ts',
      'test/foundation-errors.test.ts',
      'test/foundation-index.test.ts',
      'test/audit-service.test.ts',
      'test/entity-store-service.test.ts',
      'test/identity-service.test.ts',
      'test/session-service.test.ts',
      'test/oauth-default-clients.test.ts',
      'test/claim-policy.test.ts',
      'test/auth-broker.test.ts',
      'test/auth-broker-gate.test.ts',
      'test/auth-broker-assurance.test.ts',
      'test/federation-microsoft.test.ts',
      'test/federation-email-code.test.ts',
      'test/payment-broker.test.ts',
      'test/auth-wrap.test.ts',
    ],
    globals: true,
  },
}
